...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()){
	  ...
	  if(nHeight >= 317000){
		nSubsidy = ((rewardByYear[thisYear] * COIN)/2);
	  } else {
			...
			for (const CTxIn txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					...
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zGRPH serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zGRPH stake block is less than allowed start height", __func__);

	if (CZGrphStake* zGRPH = dynamic_cast<CZGrphStake*>(stake)) {
		CBlockIndex* pindexFrom = zGRPH->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zGRPH stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zGRPH stake does not have required confirmation depth", __func__);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zGRPH->GetValue()));
		if (nChecksum200 != zGRPH->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zGRPH->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZGRPH() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zGRPH fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zGRPH mints and %d zGRPH spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Graphcoin: We use certain sporks during IBD, so check to see if they are
